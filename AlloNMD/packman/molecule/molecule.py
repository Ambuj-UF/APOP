    ...
    for FrameNumber,frame in enumerate(frames[start:]):
        ...
        for _ in lines:
            if(_[0:4]=='ATOM'):
                ...
                Charge=_[78:80]
                AllAtoms[str(id)+ChainID]=Atom(id,AtomName,Coordinates,Occupancy,bfactor,Element,Charge,AllResidues[str(ResidueNumber)+ChainID])
                
                #Atom added to the residue
                AllResidues[str(ResidueNumber)+ChainID].__setitem__(id,AllAtoms[str(id)+ChainID])
                ...
                Element=_[76:78].strip()
                Charge=_[78:80]
                AllHetAtoms[str(AtomID)+ChainID]=Atom(AtomID,AtomName,Coordinates,Occupancy,bfactor,Element,Charge,AllHetMols[str(HetMolNumber)+ChainID])
                
                #HetAtom added to the residue
                AllHetMols[str(HetMolNumber)+ChainID].__setitem__(AtomID,AllHetAtoms[str(AtomID)+ChainID])
