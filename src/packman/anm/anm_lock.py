...
class ANM:
    ...
    def __init__(self, coords , atoms=None, gamma=1.0, dr=15.0, pert_gamma=10.0, power=0, pf=None, pocket=[], restrain=[]):
        self.gamma   = gamma
        self.dr      = dr
        self.power   = power
        self.pf      = pf
        self.pert_gamma = pert_gamma
    ...
    def calculate_hessian(self):
        ...
        gamma_normal = self.gamma
        gamma_poc = self.pert_gamma
        for i in range(len(self.coords)):
            diff = self.coords[i+1:, :] - self.coords[i] 
            squared_diff = diff**2
            for j, s_ij in enumerate(squared_diff.sum(1)):
                if (s_ij <= self.dr**2) or ((j + i + 1) in self.restrain and i in self.restrain):
                
                    if i in self.restrain and (j + i + 1) in self.restrain:
                        use_gamma = gamma_poc
                    else:
                    ...
                    diff_coords = diff[j]
                    j = j + i + 1
                    derivative = numpy.outer(diff_coords, diff_coords)*(float(-use_gamma)/numpy.sqrt(s_ij)**(2+self.power))
        ...
        if self.pf != None:
            distance_mat = distance_mat**self.pf
